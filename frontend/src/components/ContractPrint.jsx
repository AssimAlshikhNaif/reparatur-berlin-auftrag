...
import { SHOP_INFO, LIABILITY_WAIVER, AGB_FULL, DSGVO_FULL } from "@/lib/constants";
...
export default function ContractPrint({ order, branchName, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-background/80 backdrop-blur-sm flex items-start justify-center p-4 overflow-y-auto">
      {/* ستايل داخلي للتحكم بما يظهر على الشاشة وما يظهر عند الطباعة */}
      <style>{`
        @media screen {
          .print-only-page {
      ...
      <div className="bg-background border border-border max-w-3xl w-full my-6">
        ...
        <div className="p-6 bg-card flex justify-center max-h-[80vh] overflow-y-auto">
          <div id="vertrag" style={{ width: "190mm", padding: "12mm", background: "#fff", color: "#111", fontFamily: "Arial, sans-serif" }}>
            
            {/* الصفحة الأولى: تفاصيل العقد والعميل والجهاز */}
            ...
            {/* الصفحة الثانية: تظهر فقط عند الطباعة (Print) وتتضمن الشروط الكاملة والـ DSGVO */}
            <div className="print-only-page" style={{ marginTop: "30px", paddingTop: "10px" }}>
              <div style={{ fontSize: "13px", fontWeight: 700, marginBottom: "6px" }}>Anlage: Allgemeine Geschäftsbedingungen & Datenschutz</div>
              <div style={{ fontSize: "10px", color: "#666", marginBottom: "12px" }}>Zur Auftrags-Nr. {order.auftragsnummer} · {SHOP_INFO.name}</div>
              
              <div style={{ fontSize: "8px", color: "#333", lineHeight: 1.5, whiteSpace: "pre-line" }}>
                {AGB_FULL}
              </div>
              
              <div style={{ fontSize: "8px", color: "#333", lineHeight: 1.5, whiteSpace: "pre-line", marginTop: "12px" }}>
                {DSGVO_FULL}
              </div>
            </div>
