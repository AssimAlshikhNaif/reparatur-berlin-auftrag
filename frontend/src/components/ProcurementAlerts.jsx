      ...
      <div data-testid="procurement-alerts" className="border border-amber-800/60 bg-amber-950/15 rounded-xl overflow-hidden">
        <div className="flex items-center justify-between px-5 py-3 border-b border-amber-800/40 bg-amber-950/25">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-head font-semibold text-sm tracking-tight text-amber-100">Ersatzteile: Fällig heute / Überfällig</h2>
          </div>
          <div className="flex items-center gap-2 font-mono text-[10px] uppercase tracking-wider">
            {overdue > 0 && <span data-testid="alerts-overdue-count" className="px-2 py-0.5 rounded border border-red-600 bg-red-950 text-red-300">{overdue} überfällig</span>}
            {today > 0 && <span data-testid="alerts-today-count" className="px-2 py-0.5 rounded border border-amber-600 bg-amber-950 text-amber-300">{today} heute</span>}
          </div>
        </div>
          ...
          {items.map((p) => (
            <div key={p.id} data-testid={`alert-row-${p.id}`} className="flex flex-wrap items-center gap-3 px-5 py-3">
              <span className={`inline-flex items-center gap-1 px-2 py-0.5 text-[9px] font-mono uppercase tracking-wider rounded border shrink-0 ${
                ...
                {p.due_category === "OVERDUE" ? <><Warning size={11} weight="fill" /> {p.days_overdue}T überfällig</> : <><Clock size={11} weight="fill" /> heute</>}
              </span>
              <div className="min-w-0 flex-1">
                ...
                <div className="font-mono text-[10px] text-muted-foreground">
                  Erwartet: {p.expected_arrival ? berlinDateTime(p.expected_arrival) : "—"}
