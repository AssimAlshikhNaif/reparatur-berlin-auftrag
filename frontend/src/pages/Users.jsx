...
export default function Users() {
  ...
  const remove = async (u) => {
    try { await api.delete(`/users/${u.id}`); toast.success("Benutzer gelöscht"); load(); }
    catch (err) { toast.error(err.response?.data?.detail || "Fehler"); }
  };

  return (
    <div>
      <PageHeader label="Verwaltung" title="Benutzer">
        <button data-testid="add-user-button" onClick={() => setShowAdd(true)}
          ...
          <UserPlus size={16} weight="bold" /> Benutzer anlegen
        </button>
      </PageHeader>

      <div className="px-6 md:px-8 py-3 border-b border-border/60">
        <p className="text-xs font-mono text-muted-foreground">Keine öffentliche Registrierung — Konten werden ausschließlich hier vom Administrator erstellt. Demo-Passwort aller Seed-Konten: <span className="text-foreground">Repair2026!</span></p>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border text-left font-mono text-[10px] uppercase tracking-widest text-muted-foreground">
              <th className="px-6 md:px-8 py-3 font-medium">Name</th>
            ...
            {users.map((u) => {
              ...
              return (
                <tr key={u.id} data-testid={`user-row-${u.email}`} className="border-b border-border/40 hover:bg-muted transition-colors">
                  ...
                  <td className="px-4 py-3 text-muted-foreground">{u.role === "admin" ? "Alle Filialen" : branchName(u.branch_id)}</td>
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button data-testid={`copy-user-${u.email}`} onClick={() => { navigator.clipboard?.writeText(u.email); toast.success(`Kopiert: ${u.email}`); }}
                        className="p-1.5 border border-border rounded-lg hover:bg-muted text-muted-foreground" title="E-Mail kopieren"><Copy size={14} /></button>
                      {u.role !== "admin" && (
      ...
      {showAdd && (
        <div className="fixed inset-0 z-50 bg-background/80 backdrop-blur-sm flex items-center justify-center p-4">
          <form onSubmit={add} className="bg-background border border-border max-w-md w-full p-6 space-y-4">
            <h3 className="font-head font-semibold text-lg">Neuer Benutzer</h3>
            <div>
            ...
            <div className="flex gap-3">
              <button data-testid="user-save" type="submit" className="flex-1 bg-primary text-primary-foreground font-head font-semibold text-sm uppercase tracking-wider py-2.5 hover:bg-blue-600 hover:text-primary-foreground transition-colors">Anlegen</button>
              <button type="button" onClick={() => setShowAdd(false)} className="px-6 border border-border text-muted-foreground hover:text-primary-foreground hover:bg-muted transition-colors">Abbrechen</button>
            </div>
          </form>
        </div>
      )}
