...
## Iteration 16 (2026-08-16) — Real-time technician assignment notifications + regression fix
- **Targeted notifications**: `push_notification` gained `target_user_id`/`target_role`; targeted alerts fire even for admin-triggered actions. Assigning an order (`POST /orders/{id}/assign`) or creating one already assigned now emits a `kind="ASSIGNED"` notification targeted at that technician ("🔧 Neuer Auftrag zugewiesen · Ihnen wurde {nr} zugewiesen: {device}").
- **Role-scoped notification feed**: rewrote list/read/clear with `_notif_query(user)` — admin sees broadcast (target null), techniker sees only their targeted items, mitarbeiter sees branch untargeted. Endpoints now allow `techniker`. (`{target_user_id: None}` also matches legacy docs missing the field.)
- **Frontend**: NotificationBell now mounts for technicians too (Layout). It already polls every 5s, plays the ding chime, shows an unread badge + toast with an "Öffnen" link, and opens the order on click.
- **Verified**: curl — tech GET /notifications 200; assign & create-assigned → tech unread=1 with ASSIGNED alert; admin/reception don't see targeted items. Screenshot — technician dashboard shows bell + badge(1) + panel with the assignment + order link.
- **REGRESSION FIXED**: the iteration-15 delete-order edit had accidentally dropped the `@router.post("/orders")` decorator + `create_order` signature, breaking order creation (405/500). Restored; POST /orders works again. (Also note: a counters-only reset while orders remain causes duplicate auftragsnummer — always reset orders+counters together for a clean launch.)
...
## Iteration 13 (2026-06 fork) — Print/sticker layout & legal cleanup
- **Device sticker (LabelPrint.jsx)**: removed device name (brand/model) and "REPARATUR BERLIN" — sticker now contains ONLY the QR code + order number beside it (`label-order-number`). Verified via screenshot.
- **Contract cleanup (ContractPrint.jsx)**: removed the bottom "Erstellt: …" debug timestamp line (and now-unused `berlinDateTime` import). Document ends cleanly at the signature block. Verified (HAS_ERSTELLT=False).
- **Abholschein centering (index.css)**: `@media print #abholschein` now uses `left:50%; transform:translateX(-50%); margin:0 auto` so the 80mm receipt prints centered instead of left-shifted (screen preview was already flex-centered).
- **Legal text (Abholschein.jsx)**: waiver bullets kept as the concise 5-point version per user choice; cleaned li flex styling (`flex-start`, `word-break`) to guarantee no clipping. Contract already renders full LIABILITY_WAIVER points 1–5 (verified HAS_POINT5=True, not clipped).
